...
def prepare_real_data(file_path, AnchorPos):
    ...
    return len(distances), TrackTruth,PhiDiff, distances,azimuths,elevations

AnchorPos = np.array([0, 0, 1])  # Anchor coordinates

kSampleCount, TrackTruth, PhiDiff, Dis, azimuths,elevations = prepare_real_data("uwb_data_826.csv", AnchorPos)
...
np.savetxt('calibration_table_4channels_with_two.csv', calibration_table, 
           delimiter=',', 
           header=header, 
           comments='',
           fmt='%.6f')
